...
public class HBaseDAOImpl {
	...
	public HBaseDAOImpl() {
		...
		String zk_list = "node02,node03,node01";
	 ...
	 * @return
	 */
	public Result getOneRowAndMultiColumn(String tableName, String rowKey,String faimlycol, String[] cols) {
		// TODO Auto-generated method stub
		Table table = null;
		Result rsResult = null;
		try {
			table = conn.getTable(TableName.valueOf(tableName));
			Get get = new Get(rowKey.getBytes());
			byte[] colfamily = faimlycol.getBytes();
			for (int i = 0; i < cols.length; i++) {
				get.addColumn(colfamily, cols[i].getBytes());
			}
			rsResult = table.get(get);
		} catch (Exception e) {
			e.printStackTrace();
